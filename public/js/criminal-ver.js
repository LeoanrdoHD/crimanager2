...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initializeVehicleModalsSimple(); // Solo Bootstrap
...
function initializeVehicleModalsSimple() {
    console.log('🚗 Inicializando modales de vehículos (simple)...');
    
    // Encontrar todos los thumbnails que tengan data-bs-target
    ...
    // Solo necesitamos hacer que los thumbnails sean clickeables
    vehicleThumbs.forEach((thumb, index) => {
        // Hacer focuseable para accesibilidad
        thumb.setAttribute('tabindex', '0');
        thumb.setAttribute('role', 'button');
        thumb.setAttribute('aria-label', 'Ver imagen en tamaño completo');
        ...
        // Log cuando se hace click (para debug)
        thumb.addEventListener('click', function() {
            const modalTarget = this.getAttribute('data-bs-target');
            console.log(`🖱️ Click en thumbnail ${index + 1}, abriendo: ${modalTarget}`);
        });
    });
    
    console.log('✅ Thumbnails configurados - Bootstrap maneja el resto automáticamente');
}
// ========================================
