...
@section('css')
    ...
    <style>
        ...
        /* Estilos para badges de organizaciones */
        .organization-badge {
        ...
        .organization-badge:hover {
            background-color: #1e622d !important;
            color: white !important;
        }
        ...
        @media (max-width: 576px) {
            ...
            .criminal-header {
                background: linear-gradient(135deg, #000000 0%, #20c997 100%);
                color: white;
                padding: 15px;
                display: flex;
                align-items: center;
...
@section('content')
    ...
    @endif
    <!-- Tabla de criminales con organizaciones -->
    <div class="container">
        <div class="card">
            <div class="card-body">
                ...
                <div class="table-responsive-custom">
                    <table class="table" id="criminales">
                        ...
                        <tbody>
                            @php
                                // Obtener solo criminales que tienen organizaciones
                                $criminalsWithOrganizations = $crimi->filter(function ($criminal) use ($orga) {
                                    return $orga->where('criminal_id', $criminal->id)->isNotEmpty();
                                });
                            @endphp

                            @foreach ($criminalsWithOrganizations as $criminals)
                                @php
                                    // Obtener organizaciones del criminal actual - USAR values() para reiniciar índices
                                    $criminalOrganizations = $orga->where('criminal_id', $criminals->id)->values();
                                ...
                                @foreach ($criminalOrganizations as $organization_index => $criminal_organization)
                                    <tr>
                                        @if ($organization_index === 0)
                                            <!-- Solo mostrar datos básicos en la primera fila -->
                                            <td rowspan="{{ $criminalOrganizations->count() }}">{{ $criminals->id }}</td>
                                            <td rowspan="{{ $criminalOrganizations->count() }}">
                                                {{ $criminals->first_name }} {{ $criminals->last_nameP }}
                                                {{ $criminals->last_nameM }}
                                            </td>
                                            <td rowspan="{{ $criminalOrganizations->count() }}">
                                                {{ $criminals->identity_number }}</td>
                                            <td rowspan="{{ $criminalOrganizations->count() }}">
                                        ...
                                        <!-- Datos de organización (una por fila) -->
                                        <td>
                                            <span
                                                class="badge organization-badge">{{ $criminal_organization->organization->organization_name ?? 'N/A' }}</span>
                                        </td>
                                        <td>{{ $criminal_organization->organization->Criminal_Organization_Specialty ?? 'N/A' }}
                                        </td>
                                        <td>{{ $criminal_organization->criminal_rol ?? 'N/A' }}</td>

                                        @if ($organization_index === 0)
                                            <!-- Historial y botones solo en la primera fila -->
                                            <td rowspan="{{ $criminalOrganizations->count() }}">
                    ...
                    @foreach ($criminalsWithOrganizations as $criminals)
                        ...
                        <div class="criminal-card">
                            ...
                            <div class="criminal-body">
                                @foreach ($criminalOrganizations as $criminal_organization)
                                    <div class="organization-item">
                                        <div class="organization-header">
                                            <div class="organization-name">
                                                {{ $criminal_organization->organization->organization_name ?? 'N/A' }}
                                            </div>
                                        </div>

                                        <div class="organization-details">
                                            <strong>Actividad:</strong>
                                            {{ $criminal_organization->organization->Criminal_Organization_Specialty ?? 'N/A' }}<br>
                                            <strong>Rol:</strong> {{ $criminal_organization->criminal_rol ?? 'N/A' }}
                                        </div>

                                        <div class="organization-actions">
                                            <span class="text-muted small">
                                                <i class="fas fa-users"></i> Organización Criminal
                                            </span>
                                        </div>
                                    </div>
                                @endforeach
                                ...
                                @if ($criminalHistory->isNotEmpty())
                                    <div style="padding: 15px; background: #000000; border-top: 1px solid #eee;">
